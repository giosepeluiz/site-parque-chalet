...
const Mensalidade = ({ fraction }) => {
  ...
  return (
    <div className={styles["info-total"]}>
      ...
      <div className={styles["select-wrapper"]}>
        <label htmlFor="rateado">
          ...
          <input
            ...
            name="rateado"
            value={rateadas}
            onChange={(e) => setRateadas(Number(e.target.value))}
        ...
        <label htmlFor="nao-rateado">
          ...
          <input
            ...
            name="nao_rateado"
            value={naoRateadas}
            onChange={(e) => setNaoRateadas(Number(e.target.value))}
            className={`${styles.select} ${styles.currency}`}
          />
        </label>
      </div>
      ...
      />
      <MensalidadeItem
        infoItem="Valor rateado das despesas para a unidade:"
        infoValue={values.rateado}
        styles={styles}
      />
      <MensalidadeItem
        infoItem="10% do fundo de reservas (Lei Nº 4.591/64):"
        infoValue={values.reserva}
        styles={styles}
      />
      <MensalidadeItem
        infoItem="Valor do fundo de benfeitorias (AGE 09/2016):"
        infoValue={values.fundoBenfeitorias}
        styles={styles}
      />
      <MensalidadeItem
        infoItem="5% de taxa de garantidora (AGE 03/2021):"
        infoValue={values.taxaGarantidora}
        styles={styles}
      />
      <MensalidadeItem
        infoItem="Taxa de emissão de boleto (Condoplus):"
        infoValue={values.taxaBoleto}
        styles={styles}
      />
      <MensalidadeItem
